...
from src.api.dependencies import validate_model
...
async def create_chat_stream(request: ChatRequest) -> ChatStreamResponse:
    ...
    stream_id = await chat_stream_service.create_chat_stream(session_id, request.message, model)
...
async def get_chat_stream(stream_id: str) -> SSEResponse:
    ...
    queue = await chat_stream_service.subscribe_chat_stream(stream_id)
    if queue is None:

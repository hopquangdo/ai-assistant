...
from src.agents.nodes.response.prompt import RESPONSE_PROMPT
from src.agents.nodes.response.schema import ResponseOutput
from src.core.messages import strip_trailing_placeholder
...
class ResponseNode(Node):
    ...
    def _model_for(self, model: str | None):
        ...
        if resolved_model not in self._models:
            self._models[resolved_model] = llm_client_factory.get_chat_model(resolved_model).with_structured_output(
                _RESPONSE_SCHEMA
            )
    ...
    async def __call__(self, state: dict, config: RunnableConfig) -> dict:
        ...
        if not answer:
            answer = "TÃ´i Ä‘Ã£ láº¥y Ä‘Æ°á»£c dá»¯ liá»‡u, nhÆ°ng chÆ°a táº¡o Ä‘Æ°á»£c pháº§n diá»…n giáº£i."
        ...
        await dispatch_custom_event(
            StreamEvent(name=StreamEventName.NODE_COMPLETED, payload={"node": self.name, **output.model_dump()}),
            config,
        )
        return {

...
# Model cho agent duy nhất — để None để dùng model mặc định (settings.llm_model_name).
...
@lru_cache
def get_agent() -> Agent:
    """Agent ReAct duy nhất (dqh.ai_core.Agent bọc create_react_agent), nạp toàn bộ tool nghiệp vụ
    discover từ MCP server.

    lru_cache trả cùng 1 instance mỗi lần gọi — an toàn vì ALL_TOOLS được nạp 1 lần lúc app khởi
    động (xem app/main.py) trước khi có request đầu tiên. Dùng .graph để invoke/stream bất đồng bộ.
    """
    return Agent.create(tools=ALL_TOOLS, model=get_chat_model(AGENT_MODEL), system=AGENT_PROMPT)
...
def new_usage_tracker() -> tuple[Usage, UsageTracker]:
    ...
    usage = Usage(model=get_settings().llm_model_name)
